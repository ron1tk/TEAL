#!/bin/bash
#SBATCH -J teal_llama3_shared_gemm_b2_all
#SBATCH -p gpu
#SBATCH -N 1
...
#SBATCH --mem=32G
#SBATCH -t 04:00:00
#SBATCH -o logs/teal_llama3_shared_gemm_b2_all_%j.out

mkdir -p logs
mkdir -p results/llama3_shared_gemm
...
export TEAL_BATCH_SPARSE_BACKEND=shared_gemm

cd /oscar/home/rkapoor8/TEAL/gpt-fast

echo "============================================================"
echo "RUNNING SPEED BENCHMARK: shared_gemm B=2"
echo "============================================================"
...
CUDA_VISIBLE_DEVICES=0 python generate.py \
  ...
  --max_new_tokens 200 \
  --batch_size 2 \
  | tee /oscar/home/rkapoor8/TEAL/results/llama3_shared_gemm/shared_gemm_b2_speed_${SLURM_JOB_ID}.txt

echo "============================================================"
echo "RUNNING PPL EVAL: shared_gemm B=2"
echo "============================================================"
...
CUDA_VISIBLE_DEVICES=0 python eval_ppl_decode.py \
  ...
  --seq_len 128 \
  --max_batches 8 \
  | tee /oscar/home/rkapoor8/TEAL/results/llama3_shared_gemm/shared_gemm_b2_ppl_${SLURM_JOB_ID}.txt